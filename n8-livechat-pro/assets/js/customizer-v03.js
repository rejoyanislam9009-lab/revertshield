(function () {
  ...
  function renderPreview() {
    ...
    var offsetXEl = document.getElementById('n8lc-v3-offset-x');
    var offsetYEl = document.getElementById('n8lc-v3-offset-y');
    var offsetX = offsetXEl ? Math.min(48, Number(offsetXEl.value || 20) / 4) : 5;
    var offsetY = offsetYEl ? Math.min(48, Number(offsetYEl.value || 20) / 4) : 5;
    var positionEl = document.getElementById('n8lc-v3-position');
    var sideStyle = positionEl && positionEl.value === 'left' ? ('left:'+offsetX+'px;right:auto;') : ('right:'+offsetX+'px;left:auto;');
    ...
    p.innerHTML = '<div class="n8lc-v3-browser"><div class="n8lc-v3-fake-nav"><i></i><i></i><i></i></div><div class="n8lc-v3-fake-site"><b></b><span></span><span></span><div><em></em><em></em></div></div>'+
      ...
      '<div class="n8lc-v3-preview-launcher '+shape+'" style="height:'+size+'px;'+(shape==='pill'?'':'width:'+size+'px')+';'+sideStyle+'bottom:'+offsetY+'px">'+icon(iconName)+(shape==='pill'&&label?'<strong>'+esc(label)+'</strong>':'')+'</div></div>';
  }

  function render(core, visual, platform) {
    platform = platform || {};
    ...
    document.getElementById('n8lc-v3-form').addEventListener('submit', function (e) {
      ...
      var platformPayload = {
        widget_offset_x: Number(document.getElementById('n8lc-v3-offset-x').value || 0),
        widget_offset_y: Number(document.getElementById('n8lc-v3-offset-y').value || 0)
      };

      Promise.all([
        api('admin/settings', { method: 'PATCH', body: corePayload }),
        api('admin/visual-settings', { method: 'PATCH', body: visualPayload }),
        api('admin/platform/settings', { method: 'POST', body: platformPayload })
      ]).then(function () {
  ...
  Promise.all([api('admin/settings'), api('admin/visual-settings'), api('admin/platform/settings')]).then(function (r) {
    render(r[0] || {}, r[1] || {}, r[2] || {});
  }).catch(function (err) {
